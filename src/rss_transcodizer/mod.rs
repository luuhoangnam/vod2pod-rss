...
pub fn inject_vod2pod_customizations(
    ...
    // Filter items that don't meet the minimum duration threshold.
    // Items with missing or unparseable duration are also excluded.
    if let Some(min_secs) = min_duration {
        let filtered: Vec<_> = injected_feed
            .items()
            .iter()
            .filter(|item| {
                item.itunes_ext()
                    ...
                    .map(|d| d.as_secs() >= min_secs)
                    .unwrap_or(false)
            })
            .cloned()
            .collect();
        injected_feed.set_items(filtered);
    }
